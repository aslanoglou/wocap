...
document.addEventListener("DOMContentLoaded", () => {
  ...
  select.forEach(selectElement => {
    selectElement.addEventListener('change', function () {
      if (this.value === "") {
        this.classList.remove('text-(--wocap-gray-800)');
        this.classList.add('text-(--wocap-gray-600)');
      } else {
        this.classList.remove('text-(--wocap-gray-600)');
        this.classList.add('text-(--wocap-gray-800)');
      }
    });
  });
